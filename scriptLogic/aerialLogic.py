...
def requestGeoInformation(locationFromUser):
    ...
    params = {
        'searchText': locationFromUser,  # Die Adresse, die geocodiert werden soll
    ...
    if response.status_code == 200:
        ...
        if data['results']:
            ...
            print(f'Für den Ort "{locationFromUser}" kennt GeoAdmin leider keinen Eintrag.')
            return '', '', ''
    else:
        print(f"Für den Ort {locationFromUser} ist folgender Fehler aufgetreten:")
        print(f"{response.status_code} - {response.text}")
        return '', '', ''


# Get aerial image for coordinates and save to given output folder with given file name
def requestAndSaveImage(x, y, fileName, outputFolder, subfolder, zoomLevel):
    # Check logs to stay within API limits and wait if necessary
    safeRequest()
    ...
    if response.status_code == 200:
        ...
        os.makedirs(f'./{outputFolder}/{subfolder}', exist_ok=True)

        # Save image
        with open(f"./{outputFolder}/{subfolder}/{fileName}.jpg", "wb") as f:
            f.write(response.content)
        print(f"Luftbild für {fileName} erfolgreich heruntergeladen.")
        print(f"Es wird nach Ende des Skripts im Ordner ./{outputFolder}/{subfolder}/ gespeichert.")
        print()

    # Case: Didn't get results
    else:
        print(f"Die Luftbildanfrage für {fileName} gab folgenden Fehler: {response.status_code} - {response.text}")
